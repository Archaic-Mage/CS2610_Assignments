...
long long int recent_colacc[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];
long long int recent_closed[MAX_NUM_CHANNELS][MAX_NUM_RANKS][MAX_NUM_BANKS];


/* Keeping track of how many preemptive precharges are performed. */
long long int num_aggr_precharge = 0;

unsigned short int counter = 8;
...
void schedule(int channel)
{
	...
	if(counter > HI_THRES) policy = 1;
	if(counter < LO_THRES) policy = 0;
			...
			LL_FOREACH(write_queue_head[channel], wr_ptr)
			{
				if (wr_ptr->command_issuable)
				{
					...
					if (wr_ptr->next_command == COL_WRITE_CMD)
					{
						...
						if(recent_closed[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] == wr_ptr->dram_addr.row+1) counter = (counter == 0) ? 0: counter-1;
					}
			...
			LL_FOREACH(read_queue_head[channel], rd_ptr)
			{
				if (rd_ptr->command_issuable)
				{
					...
					if (rd_ptr->next_command == COL_READ_CMD)
					{
						...
						if(recent_closed[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] == rd_ptr->dram_addr.row+1) counter = (counter == 0) ? 0: counter-1;
					}
				...
				{ /* For all banks on the channel.. */
					...
					{ /* See if this bank is a candidate. */
						...
						{ /* See if precharge is doable. */
							...
							{
								num_aggr_precharge++;
								recent_closed[channel][i][j] = recent_colacc[channel][i][j];
		...
		if (drain_writes[channel])
		{
			...
			LL_FOREACH(write_queue_head[channel], wr_ptr)
			{
				if (wr_ptr->command_issuable)
				{
					...
					if(dram_state[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank].active_row != wr_ptr->dram_addr.row) counter = (counter == 15) ? 15: counter+1; //saturation counter
					issue_request_command(wr_ptr);
					break;
				}
			}
		...
		if (!drain_writes[channel])
		{
			LL_FOREACH(read_queue_head[channel], rd_ptr)
			{
				if (rd_ptr->command_issuable)
				{
					...
					if(dram_state[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank].active_row != rd_ptr->dram_addr.row) counter = (counter == 15) ? 15: counter+1; //saturation counter
					issue_request_command(rd_ptr);
					break;
				}
			}
...
void scheduler_stats()
{
	...
	if(policy) printf("Number of aggressive precharges: %lld\n", num_aggr_precharge);
}
